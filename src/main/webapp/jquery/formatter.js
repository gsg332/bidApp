...
function formatContBizInfo4(val, row){
	return Math.ceil((parseInt(row.cnt)/parseInt(row.cont_biz_num))*1000)/10;
}						        
...
function test(n){
	...
	sum();
	return $(n).val(numberComma($(n).val().replaceAll(",",""),","));
}

function sum(){
	var base = eval($("#base_price").val().replaceAll(",",""));
	var cont_point = eval($("#cont_point").val().replaceAll(",",""));
